...
const LandingPage: React.FC<{ onConnect: () => void }> = ({ onConnect }) => (
  <div className="min-h-screen bg-gray-50 dark:bg-darkbg flex flex-col items-center justify-center p-4 font-sans transition-colors duration-200">
    <div className="max-w-md w-full bg-white dark:bg-panel border border-gray-200 dark:border-gray-800 rounded-xl p-8 shadow-2xl text-center transition-colors">
      ...
      <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">市政AI生图工具</h1>
      <p className="text-gray-600 dark:text-gray-400 mb-8">专业的市政基础设施可视化平台。需要连接 Google Cloud 项目以进行高保真渲染。</p>
      ...
      >
        <span>连接 Google Cloud 项目</span>
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7l5 5m0 0l-5 5m5-5H6" /></svg>
      </button>
      
      <p className="mt-6 text-xs text-gray-500 leading-relaxed">
        本工具使用 <strong>gemini-3-pro-image-preview</strong> 模型。
        <br />
        您必须选择一个已启用计费的项目。 
        <a href="https://ai.google.dev/gemini-api/docs/billing" target="_blank" rel="noreferrer" className="text-municipal-600 dark:text-municipal-500 hover:underline ml-1">了解更多</a>
      </p>
    </div>
  </div>
);
...
const App: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Ensure we have a key before generating (double check for robustness)
      if (!hasApiKey && window.aistudio) {
         const hasKey = await window.aistudio.hasSelectedApiKey();
         if (!hasKey) {
             throw new Error("API Key 未选择。请先连接项目。");
         }
      }
             ...
             <div className="space-y-2">
               {project.versions.map(v => (
                 <div 
                   ...
                   className={`p-2 rounded cursor-pointer text-sm flex gap-2 items-center border transition-colors ${
          ...
          <div className="p-4 space-y-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                />
                <p className="text-[10px] text-gray-400 dark:text-gray-500 mt-1">数值越高，越严格遵循原始线条。</p>
              </div>
            ...
            <div className="space-y-3">
               <div className="flex items-center justify-between">
                  ...
                  <select 
                    ...
                    onChange={(e) => setParams(p => ({...p, outputQuality: e.target.value as '1K'|'2K'}))}
            ...
            <div className="mt-8 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-900/50 rounded text-xs text-blue-800 dark:text-blue-200">
               <strong>当前模型:</strong> Nano Banana Pro
               <br/>
               <span className="opacity-70">专为结构完整性和照片级渲染优化。</span>
            </div>
